...
def test_createWordList_1():
  words = ['player', 'football', 'spiderman']
...
def test_canWeMakeIt_1():
  assert (canWeMakeIt('baller','rlleabe') == True)

def test_canWeMakeIt_2():
  assert (canWeMakeIt('zach','sara') == False)
...
def test_getWordPoints_1():
  assert (getWordPoints('zach', letterPoints) == 18)

def test_getWordPoints_2():
  assert (getWordPoints('dog', letterPoints) == 5)
